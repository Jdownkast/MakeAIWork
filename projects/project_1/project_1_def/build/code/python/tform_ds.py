...
# Some code is added to create a new 'type' of datasets, see comments on top of the notebook
...
for new in [False, True]:
    ...
    # All datasets, old and new, are build from the database
    # The new datasets are afterwards altered

    # The variables in de database are numbered as follows:
    # 0: lifespan, 1: genetics, 2: length, 3: mass, 4: exercise,
    # 5: smoking, 6: alcohol, 7: sugar, 9 & 10: normalized bmi
    df_dict = {
    ...
    for dataset in df_dict:
        df_dict[dataset].to_csv(
            f"{path_dataset}{new_}{ds_name}{str(dataset)}{extension}", index=False
        )
